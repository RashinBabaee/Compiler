 ...
 * -------------------------------------------------------------
 */
rs_void bErrorPrint(rs_char* fmt, ...);
rs_void displayBuffer(BufferReader* ptr_Buffer);
rs_long getFileSize(rs_char* fname);
rs_intg isNumber(const rs_char* ns);
rs_void startReader(rs_char*, rs_char*, rs_char, rs_intg, rs_intg);
...
rs_intg mainReader(rs_intg argc, rs_char** argv) {

	/* Create source input buffer */
	rs_char* program = argv[0];
	rs_char* input = argv[2];
	rs_char mode = MODE_FIXED;
	rs_intg size = 0, increment = 0, wrongNumber = 0;
...
************************************************************
*/
rs_void startReader(rs_char* program, rs_char* input, rs_char mode, rs_intg size, rs_intg increment) {

	ReaderPointer bufferp;		/* pointer to Buffer structure */
	FILE* fileHandler;			/* input file handle */
	rs_intg loadSize = 0;		/* the size of the file loaded in the buffer */
	rs_char symbol;			/* symbol read from input file */
...
rs_void displayBuffer(BufferReader* ptr_Buffer) {
	...
	if (!readerPrint(ptr_Buffer))
		printf("Empty buffer\n");
	printf("wrte:%d\nread:%d\nmark:%d\n", ptr_Buffer->position.wrte, ptr_Buffer->position.read, ptr_Buffer->position.mark);
	printf("%x\n", ptr_Buffer->flags);
}
