...
extern rs_intg startScanner(ReaderPointer sc_buf);

static rs_void printParserError(rs_char* fmt, ...);
static rs_void displayParser(ReaderPointer ptrBuffer);
static rs_long getParserFileSize(rs_char* fname);
...
rs_intg mainParser(rs_intg argc, rs_char** argv) {
...
rs_void printParserError(rs_char* fmt, ...) {
...
rs_long getParserFileSize(rs_char* fname) {
	FILE* input;
	rs_long flength;
	input = fopen(fname, "r");
	if (input == NULL) {
...
rs_void displayParser(ReaderPointer ptrBuffer) {
